              ...
              int              index)
{
  ...
  switch (type)
    ...
    case GIMP_PDB_STRING:
      ...
      // because they returned NULL values from here (some return values are explicitely missing)
      // Do we need cheks on other GimpParamData union members ? (see gimp.h)
      if (param->data.d_string != 0)
...
/*    case GIMP_PDB_REGION:
    ...
    case GIMP_PDB_ITEM:
      data = INT2NUM(param->data.d_item);
      break;
...
rb2GimpParam (VALUE rbparam)
{
  ...
  switch (type)
    ...
    case GIMP_PDB_ITEM:
      result.data.d_item = (gint32)NUM2INT(rbdata);
      break;
