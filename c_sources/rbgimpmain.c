...
handle_exception (char *message)
{
  ...
  str = StringValuePtr(rbmessage);
  
  result = g_strconcat(message, str, NULL);
  gimp_message(result);
  g_free(result);
...
init_callback (void)
{
  ...
  if (err)
    handle_exception(RSTRING_PTR("An exception has occurred while running the init callback."));
}
...
quit_callback (void)
{
  ...
  if (err)
    handle_exception(RSTRING_PTR("An exception has occurred while running the quit callback."));
}
...
query_callback (void)
{
  ...
  if (err)
    handle_exception(RSTRING_PTR("An exception has occurred while running the query callback"));
}
              ...
              GimpParam        **return_vals)
{
  ...
  if (err)
    handle_exception(RSTRING_PTR("An exception has occured while running the run callback"));
...
static VALUE
rb_gimp_main(VALUE self, VALUE plug_in_info)
{
