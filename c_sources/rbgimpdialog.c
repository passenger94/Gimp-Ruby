...
static void
ruby_fu_string_update(gchar **dest, const gchar  *src)
{
  if (*dest) g_free(*dest);
  *dest = g_strdup(src);
}
...
static void
ruby_fu_gradient_callback(gpointer       data,
                          const gchar   *name,
                          gint           width,
                          const gdouble *mask_data,
                          gboolean       closing)
{
  ruby_fu_string_update(data, name);
}

*******************************************************************************/

#define WIDGET2NAME(w, W) \
  static VALUE \
  get_name_from_##w##_widget(gpointer widget_ptr) \
  { \
    gchar *str; \
    volatile VALUE rbstr; \
    str = (gchar *)gimp_##w##_select_button_get_##w(GIMP_##W##_SELECT_BUTTON(widget_ptr)); \
    rbstr = rb_str_new2(str); \
    g_free(str); \
    return rbstr; \
  }

WIDGET2NAME(palette, PALETTE);
WIDGET2NAME(gradient, GRADIENT);
WIDGET2NAME(pattern, PATTERN);

static VALUE
get_name_from_brush_widget(gpointer widget_ptr)
{
  gchar *str;
  volatile VALUE rbstr;
  gdouble opacity;
  ...
  str = (gchar *)gimp_brush_select_button_get_brush(GIMP_BRUSH_SELECT_BUTTON(widget_ptr), 
...
make_list_combo_box (VALUE list)
{
  ...
  int num = RARRAY_LENINT(list);
  VALUE *ary = RARRAY_PTR(list);
                     ...
                     Result *result)
{
  ...
  if (subtypeid == rb_intern("text"))
    ...
    {
      widget = gimp_palette_select_button_new("Ruby-Fu", defstr);
        
      result->ptr = widget;
      result->func = &get_name_from_palette_widget;
    }
//  else if (subtypeid == rb_intern("gradient"))
//    {
//      widget = gimp_gradient_select_button_new("Ruby-Fu", defstr);
//      g_signal_connect_swapped (widget, "gradient-set",
//                                G_CALLBACK (ruby_fu_gradient_callback),
//                                &defstr);
//      
...
//    }
  else if (subtypeid == rb_intern("gradient"))
    {
      ...
      result->ptr = widget;
      result->func = &get_name_from_gradient_widget;
    }
  else if (subtypeid == rb_intern("pattern"))
    {
      widget = gimp_pattern_select_button_new("Ruby-Fu", defstr);
  
      result->ptr = widget;
      result->func = &get_name_from_pattern_widget;
    }
  else if (subtypeid == rb_intern("brush"))
    {
      widget = gimp_brush_select_button_new("Ruby-Fu", defstr, 100, -1, GIMP_NORMAL_MODE);
      
      result->ptr = widget;
      result->func = &get_name_from_brush_widget;
    }
            ...
            Result **results)
{
  ...
  int num = RARRAY_LENINT(params);
  VALUE *ary = RARRAY_PTR(params);
