...
RubyFu.register(
  ...
  :results    => [] #results
	...
	Context.push do
		...
		bglayer = Layer.new(img, xsize, ysize, RGB, "Background", 100, NORMAL_MODE)
		
		img.undo_group_start do
			
			img.add_layer(bglayer, 1)
			
			Context.set_background(bgcolor)
			Edit.fill(bglayer, BACKGROUND_FILL)
			...
			if shadow == 1
				...
				0.upto(5) do |i|
					gimp_rect_select(img, pos[i][:x]+20, pos[i][:y]+20, 1024, 768, 0, 1, 20.0)
				end
