	...
	Context.push do
		image.undo_group_start do
			...
			highlight = Layer.new(image, lwidth, lheight, RGBA_IMAGE, "highlight", 100,
														NORMAL_MODE)
			[shadow, gradient, highlight].each {|l| l.fill(TRANSPARENT_FILL); image.insert_layer(l, nil, -1)}
			
			Context.set_foreground(darken(c2))
			Edit.bucket_fill(shadow, FG_BUCKET_FILL, NORMAL_MODE, 100, 0, false, 0, 0)
			
			Context.set_foreground(c1)
			Context.set_background(c2)
			Edit.blend(gradient, FG_BG_RGB_MODE, NORMAL_MODE, GRADIENT_LINEAR, 100, 0,
			...
			Edit.blend(highlight, FG_TRANSPARENT_MODE, NORMAL_MODE, GRADIENT_LINEAR, 100, 0,
