...
def apply_cool_metal_logo_effect(img, logo_layer, size, bg_color, gradient, gradient_reverse)
    ...
    Edit.fill(bg_layer, BACKGROUND_FILL)
    reflect_layer.add_alpha
    Edit.clear reflect_layer
    Context.set_background Color(0,0,0)
    Edit.fill(shadow_layer, BACKGROUND_FILL)
   
    Context.set_gradient gradient
    
    Edit.blend(logo_layer, CUSTOM_MODE, NORMAL_MODE, GRADIENT_LINEAR, 100, 0, 
    ...
    Selection.invert img
    Context.set_background Color(0,0,0)
    Edit.fill(channel, BACKGROUND_FILL)
    Selection.none img
    ...
    layer_mask = reflect_layer.addMask ADD_WHITE_MASK
    Context.set_foreground Color(1.0,1.0,1.0)
    Context.set_background Color(0,0,0)
    Edit.blend(layer_mask, FG_BG_RGB_MODE, NORMAL_MODE, GRADIENT_LINEAR, 100, 0, 
