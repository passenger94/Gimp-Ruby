...
RubyFu.register(
    ...
    Context.push do
        image.undo_group_start do
            ...
            bump = image.addLayer_from_drawable(drawable, image.get_item_position(orig))
            bump.set_name("bump")
            bump.set_visible(false)
            Context.set_foreground(Color(1.0,1.0,1.0))
            image.select_item(CHANNEL_OP_REPLACE, bump)
            Edit.bucket_fill(bump, BUCKET_FILL_FG, NORMAL_MODE, 100, 0, false, 0, 0)
            
            #col.rgba_set((col.r - 1).abs, (col.g - 1).abs, (col.b - 1).abs, 1)
            Context.set_foreground(col.subtract(Color(1,1,1))* -1)
            Edit.bucket_fill(drawable, BUCKET_FILL_FG, NORMAL_MODE, 100, 0, false, 0, 0)
            ...
            drawable.curves_spline(HISTOGRAM_ALPHA, 4, [0.0, 0.0, 255.0, translucency.to_f])
